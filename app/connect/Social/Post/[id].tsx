...
const Post = () => {
  ...
  const getImages = useCallback(async() => {
    // @ts-ignore
    const { data, error } = await supabase.storage.from('postPictures').list(id)
      ...
      <Carousel
          ...
          renderItem={({ item }) => (
              <View style={{ padding: 5 }}>
                <Image borderRadius={50} src={`https://gnmpzioggytlqzekuyuo.supabase.co/storage/v1/object/public/postPictures/${id}/${item.name}`} alt={item.name} style={{ width: '100%', height: '95%', objectFit: 'cover'}} />
              </View>
          )}
      />
      )}
